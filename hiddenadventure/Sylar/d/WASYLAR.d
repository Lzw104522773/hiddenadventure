...
IF ~~ THEN BEGIN 1
	...
	IF ~!Kit(LastTalkedToBy(Myself),MAGESCHOOL_NECROMANCER)~ THEN REPLY @6 GOTO 3
...
IF ~~ THEN BEGIN 4
	...
	IF ~Kit(LastTalkedToBy(Myself),MAGESCHOOL_NECROMANCER)~ THEN REPLY @14 GOTO 7
END
...
IF ~~ THEN BEGIN 7
	SAY @17 
	IF ~~ THEN DO ~SetGlobal("WAHelpSylar","Global",1)
	AddJournalEntry(@9005,INFO)~ EXIT
END
...
IF ~~ THEN BEGIN 10
	SAY @21
	IF ~~ THEN DO ~SetGlobal("WAHelpSylar","Global",1)
	AddJournalEntry(@9006,INFO)~ EXIT
END

IF ~Global("WAHelpSylar","Global",1)~ THEN BEGIN 11
	SAY @22
	IF ~~ THEN REPLY @23 EXIT
	IF ~PartyHasItem("Misc80")~ THEN REPLY @24 GOTO 12
END

IF ~~ THEN BEGIN 12
	SAY @25
	IF ~~ THEN DO ~TakePartyItem("MISC80")
	GiveGoldForce(150)
...
IF ~Global("WAHelpSylar","Global",2)~ THEN BEGIN 13
	SAY @26
	IF ~~ THEN REPLY @23 EXIT
	IF ~PartyHasItem("Misc80")~ THEN REPLY @24 GOTO 14
END
...
IF ~Global("WAHelpSylar","Global",3)~ THEN BEGIN 15
	SAY @26
	IF ~~ THEN REPLY @23 EXIT
	IF ~PartyHasItem("Misc80")~ THEN REPLY @24 GOTO 16
END
...
IF ~Global("WAHelpSylar","Global",4)~ THEN BEGIN 17
	SAY @26
	IF ~~ THEN REPLY @23 EXIT
	IF ~PartyHasItem("Misc80")~ THEN REPLY @24 GOTO 18
END
...
IF ~Global("WAHelpSylar","Global",6)~ THEN BEGIN 21
	SAY @26
	IF ~~ THEN REPLY @23 EXIT
	IF ~PartyHasItem("Misc80")~ THEN REPLY @24 GOTO 22
END
